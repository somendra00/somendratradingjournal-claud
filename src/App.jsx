...
/* ---------------------------------------------------------------
   TOKENS — LegendJournal palette
   bg        #0A0714  (background)
   surface   #15121F  (surface 1)
   surface-2 #1E1B2E  (surface 2)
   line      rgba(168,85,247,0.16)
   ink       #F5F3FF  (text primary)
   ink-dim   #A1A1AA  (text secondary)
   violet    #A855F7  (primary — used as the app's main accent)
   secondary #38BDF8
   accent    #EC4899  (paired with primary in gradient CTAs)
   lime      #10B981  (success / profit)
   coral     #EF4444  (danger / loss)
   amber     #F59E0B  (warning / featured / gold highlights)
...
/* LegendJournal palette */
const COLORS = {
  bg: "#0A0714",
  surface: "#15121F",
  surface2: "#1E1B2E",
  line: "rgba(168,85,247,0.16)",
  lineSoft: "rgba(168,85,247,0.08)",
  ink: "#F5F3FF",
  inkDim: "#A1A1AA",
  inkFaint: "#6B7280",
  violet: "#A855F7",
  ...
  lime: "#10B981",
  limeSoft: "rgba(16,185,129,0.12)",
  coral: "#EF4444",
  coralSoft: "rgba(239,68,68,0.12)",
  amber: "#F59E0B",
  secondary: "#38BDF8",
  accent: "#EC4899",
};

const GRADIENT_PRIMARY = "linear-gradient(90deg, #A855F7 0%, #EC4899 100%)";
...
function JournalView({ onSave, currentBalance, withdrawals, onAddWithdrawal, onDeleteWithdrawal }) {
  ...
  const submit = (e) => {
    ...
    if (!assetValue) {
      setFormError(form.asset === "Other" ? "Enter a custom asset name." : "Choose an asset.");
      return;
    }
    if (form.pnl === "" || isNaN(parseFloat(form.pnl))) {
      setFormError("Enter a profit or loss amount.");
      return;
    }
    setFormError(null);
        ...
        <div className="rounded-2xl p-6" style={{ background: "var(--glass-card-bg)", border: "1px solid var(--glass-card-border)", backdropFilter: "var(--glass-card-blur)", WebkitBackdropFilter: "var(--glass-card-blur)" }}>
          ...
          <form onSubmit={submit} className="flex flex-col gap-5">
            ...
            <button
              ...
              style={{ background: GRADIENT_PRIMARY, color: "#04121a", fontFamily: FONT_BODY }}
            >
              Save Trade
            </button>
            ...
            {formError && (
              <div className="text-xs text-center py-2 rounded-lg" style={{ background: COLORS.coralSoft, color: COLORS.coral, fontFamily: FONT_BODY }}>
                {formError}
              </div>
            )}
          </form>
...
function BrokerConnectCard() {
  ...
  const submit = async (e) => {
    ...
    if (!form.login || !form.password || !form.server) {
      setError("Please fill in login, password, and server before connecting.");
      return;
    }
    setConnecting(true);
    setError(null);
      ...
      <div className="rounded-2xl p-6" style={{ background: "var(--glass-card-bg)", border: "1px solid var(--glass-card-border)", backdropFilter: "var(--glass-card-blur)", WebkitBackdropFilter: "var(--glass-card-blur)" }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <button
            ...
            className="text-left p-4 rounded-xl transition-colors hover:bg-white/[0.03]"
            style={{ background: COLORS.violetSoft, border: `1px solid rgba(168,85,247,0.35)` }}
          >
    ...
    <>
      ...
      >
        ...
        <form onSubmit={submit} className="flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-5">
          ...
          <button type="submit" className="mt-2 py-3 rounded-xl text-sm font-semibold transition-opacity hover:opacity-90" style={{ background: GRADIENT_PRIMARY, color: "#04121a", fontFamily: FONT_BODY }}>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [trades, settings, withdrawalsList] = await withTimeout(
          Promise.all([api.getTrades(), api.getSettings(), api.getWithdrawals()]),
          8000
        );
        if (cancelled) return;
        setLog(trades);
        ...
        <div className="flex items-center justify-between mb-7">
          ...
          {active !== "Journal" && (
            <button
              ...
              style={{ background: GRADIENT_PRIMARY, color: "#04121a" }}
            >
